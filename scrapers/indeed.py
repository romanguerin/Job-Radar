...
class IndeedScraper(JobScraper):
    ...
    async def fetch_jobs(self, search_terms: list[str]) -> list[ScrapedJob]:
        ...
        async with async_playwright() as playwright:
            ...
            try:
                for term in terms[:3]:
                    search_url = f"{self.base_url}/jobs?q={quote_plus(term)}"
                    await page.goto(search_url, wait_until="domcontentloaded", timeout=20000)
                    cards = page.locator("[data-testid='slider_item'], .job_seen_beacon, .result")
                    count = min(await cards.count(), 15)
                    for index in range(count):
                        card = cards.nth(index)
                        title = await safe_text(card.locator("h2 a span, h2 span, .jobTitle"), term.title())
                        company = await safe_text(card.locator("[data-testid='company-name'], .companyName"), "")
                        location = await safe_text(card.locator("[data-testid='text-location'], .companyLocation"), "")
